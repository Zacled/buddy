/*
 * main-world.js  (runs in the PAGE on wheelofnames.com)
 *
 * Press a number key to choose the winner by position:
 *   1 -> 1st name, 2 -> 2nd name, ... 9 -> 9th name, 0 / Esc -> fair spin.
 *
 * It lands on the chosen name but at a RANDOM spot within that slice, so the
 * wheel stops somewhere different every time.
 *
 * The site's winner is: index = round( N * ((u + delta) mod 1) ) (measured from
 * real spins; delta ≈ 0.363). So to land position t we force the RNG to return
 * u = (t/N + jitter − delta) mod 1, only at the moment you spin.
 */
(function () {
  "use strict";
  ...
  let forceThisSpin = false, curK = 0, mathUntil = 0;
  ...
  function onSpin() {
    ...
    const N = readEntries().length, t = state.forceIndex;
