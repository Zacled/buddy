...
(function () {
  ...
  function showView(view) { // "lock" | "warn" | "main"
    ...
    logoutEl.hidden = view !== "main";
    if (view !== "main") { document.body.classList.remove("warned"); warnBanner.hidden = true; }
  }

  // Log out: forget the stored code + acknowledged-warning state and return to
  // the activation (enter-code) screen. The device binding lives server-side, so
  // re-entering the same code on THIS machine still works; it's still blocked on
  // any other machine.
  function logout() {
  ...
  function setWarnBanner(n) {
    if (n > 0) {
      ...
      warnBanner.textContent = "⚠ Warning " + n + " of 3 — your code is active on another device. At 3 it auto-revokes for everyone.";
  ...
  async function activate() {
    ...
    if (result.status === "dead") return lock("This code has been auto-revoked (used on too many devices).");
    if (result.status === "blocked") return lock("This code is already active on another device.");
    // New code accepted on this device — clear any leftover warning ack.
    chrome.storage.local.set({ licenseCode: code, ackWarn: 0 }, () => enterMain(0));
  }
  ...
  function refreshState() {
    ...
    chrome.storage.local.get(["licenseCode", "deviceId", "ackWarn"], async (c) => {
      ...
      if (st.state === "dead") return lock("This code has been auto-revoked (used on too many devices).");
      if (st.state === "blocked") return lock("This code is already active on another device.");
